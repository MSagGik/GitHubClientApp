...
class OauthSearchFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_search_oauth, container, false)
                        ...
                        if (response.code() == 200) {
                            if (response.body()?.isNotEmpty() == true) {
                                ...
                                item.numberFollowers =
                                    if (listFollowers.size == 1) {
                                        "${listFollowers.size} ${getString(R.string.follower)}"
                                    } else if (listFollowers.size in 2..99) {
                                        "${listFollowers.size} ${getString(R.string.followers)}"
                                    } else if (listFollowers.size > 99) {
                                        getString(R.string.more_than_99_followers)
                                    } else {
                                        ""
                                    }
                                listItemAdapter.notifyDataSetChanged()
    ...
    private fun showLocation(item: Item) {
        if (item.login?.isNotEmpty() == true) {
            gitHubRestService.showUserOauth(token, item.login!!).enqueue(object :
                Callback<User> {
                @SuppressLint("NotifyDataSetChanged")
                override fun onResponse(
